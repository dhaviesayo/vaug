...
def resize(
    ...
    for video in vid:
        ...
        videos_out.append(frames_stacked)
    return videos_out
...
def perspective(
    vid_batch: Tensor, distortion_scale = 0.5, p=0.5 , interpolation: str = "bilinear", fill: Optional[list[float]] = None):

    assert type(vid_batch) ==  list or type(vid_batch) == torch.Tensor

    if type(vid_batch) == torch.Tensor:
        assert len(vid_batch.shape) == 5
    elif type(vid_batch) ==  list :
        assert type(vid_batch[0:]) == torch.Tensor
    else:
        raise TypeError( "input must be either list of video tensors or batched video tensor ")


    def rand_coef(
    height,width,distortion_scale=distortion_scale
    ):  
    ...
    def get_perspective_coeffs(startpoints , endpoints ) -> list[float]:
        ...
        res = torch.linalg.lstsq(a_matrix, b_matrix ).solution  #a_matrix is broadcasted to b_matix    a_matrix = 1,8 ;  b_matrix = 4,2

        return res.tolist()   # 8,8

    vidtransformed = []
    for vid in vid_batch:
        startpoints , endpoints = rand_coef(vid.shape[-2] , vid.shape[-1])
        coef = get_perspective_coeffs( startpoints , endpoints )
        frames_applied = []
        for frame in vid:
            _assert_image_tensor(frame)
            ...
            ow, oh = frame.shape[-1], frame.shape[-2]
            dtype = frame.dtype if torch.is_floating_point(vid[0]) else torch.float32
            grid = _perspective_grid(coef, ow=ow, oh=oh, dtype=dtype, device=frame.device)
            applied = _apply_grid_transform(frame, grid, interpolation, fill=fill)
            frames_applied.append(applied)
        vidtransformed.append(torch.stack(frames_applied))
    frames_applied.clear()
    return vidtransformed
